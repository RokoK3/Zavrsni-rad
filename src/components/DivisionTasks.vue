<template>
  ...
  export default {
    ...
    methods: {
      async fetchTasks() {
        this.tasks = [];
        await this.fetchTask("http://localhost:8080/dijeljenje/1");
        await this.fetchTask("http://localhost:8080/dijeljenje/2");
        await this.fetchTask("http://localhost:8080/dijeljenje/3");
      },
      async fetchTask(url) {
        try {
