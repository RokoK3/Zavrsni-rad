<template>
  <div class="addition-tasks">
    ...
    <h1>Uspoređivanje brojeva do milijun i mjesna vrijednost znamenaka</h1>
...
export default {
  ...
  methods: {
    async fetchTasks() {
      this.tasks = [];
      await this.fetchTask("http://localhost:8080/dekadskeZnamenke/1");
      await this.fetchTask("http://localhost:8080/dekadskeZnamenke/2");
      await this.fetchTask("http://localhost:8080/dekadskeZnamenke/3");
    },
    async fetchTask(url) {
      try {
