  ...
  <div class="addition-tasks">
    ...
    <div class="task-container" v-if="!done">
      <div class="task" v-if="tasks.length > 0">
        ...
        <div class="hint-icon" @mouseover="tasks[currentTaskIndex].showHintTooltip = true" @mouseout="tasks[currentTaskIndex].showHintTooltip = false"
         @click="tasks[currentTaskIndex].showHintText = !tasks[currentTaskIndex].showHintText">
...
export default {
  ...
  methods: {
    async fetchTasks() {
      this.tasks = [];
      await this.fetchTask("http://localhost:8080/mnozenje/1");
      await this.fetchTask("http://localhost:8080/mnozenje/2");
      await this.fetchTask("http://localhost:8080/mnozenje/3");
      await this.fetchTask("http://localhost:8080/mnozenje/4");
    },
    async fetchTask(url) {
      try {
